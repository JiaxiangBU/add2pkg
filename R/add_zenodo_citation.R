...
add_zenodo_citation <-
    function(readme_path = here::here("../pyks/README.Rmd")) {
        ...
        html <- read_html("https://zenodo.org/badge/latestdoi/197245838")
        record_id <-
            html %>% html_text() %>% str_match("zenodo.(\\d+)") %>% .[1, 2]
        record_url <-
            glue::glue("https://zenodo.org/record/{record_id}/export/hx")
        library(rvest)
        html <- read_html(record_url)
        bibtex_text <- html %>% html_nodes("pre") %>% html_text() %>% .[1]
